...
export default function BillingPage() {
  ...
  const openPreview = (doc: any, type: 'INVOICE' | 'QUOTATION') => {
    if (type === 'QUOTATION') {
      router.push(`/dashboard/quotations?id=${doc.id || doc.bookingId}`);
      return;
    }
    setPreviewDoc(doc);
    setPreviewType(type);
  ...
  return (
    <div className="space-y-4">
      ...
      {previewDoc && (
        <div className="hidden print:block bg-white text-neutral-800 w-full font-sans">
          {previewType === 'QUOTATION' ? (
            <QuotationTemplate doc={previewDoc} />
          ) : (
            <div className="p-8 space-y-4 text-xs leading-relaxed">
              {/* Logo / Company Header */}
              <div className="flex justify-between items-start border-b-2 border-primary-500 pb-4">
                ...
                <div className="text-right">
                  <h2 className="text-lg font-bold text-primary-600 tracking-wider">TAX INVOICE</h2>
                  <p className="font-semibold text-neutral-700 mt-1">{previewDoc.invoiceNumber}</p>
              ...
              <div className="grid grid-cols-2 gap-5 text-[11px]">
                ...
                <div>
                  ...
                  <p className="text-neutral-500">HSN Code: {previewDoc.hsnCode || '998381'}</p>
                </div>
              </div>
        ...
        <div className="print:hidden fixed inset-0 z-50 bg-neutral-900/30 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded max-w-3xl w-full shadow-lg border border-primary-100 flex flex-col h-[90vh] animate-scaleIn">
            ...
            <div className="flex-1 overflow-y-auto p-8 bg-neutral-100/50 flex justify-center">
              {previewType === 'QUOTATION' ? (
                <div id="pdf-document" className="w-full max-w-[760px]">
                  <QuotationTemplate doc={previewDoc} />
                </div>
              ) : (
                <div id="pdf-document" className="bg-white p-10 w-full max-w-[700px] shadow-sm border border-neutral-200/50 space-y-5 text-xs text-neutral-700 leading-relaxed font-sans min-h-[842px]">
                  ...
                  <div className="flex justify-between items-start border-b-2 border-primary-500 pb-4">
                    ...
                    <div className="text-right">
                      ...
                      <p className="font-bold text-neutral-700 text-xs mt-0.5">{previewDoc.invoiceNumber}</p>
