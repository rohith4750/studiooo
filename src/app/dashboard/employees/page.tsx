  ...
  return (
    <div className="space-y-4 animate-fadeIn">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold tracking-tight text-neutral-800">Staff & Payroll Roster</h2>
          <p className="text-xs text-neutral-500 mt-0.5">Manage studio employee profiles, functional roles, and salaries ledger.</p>
        </div>
        <button onClick={() => router.push('/dashboard/employees/create')} className="inline-flex items-center space-x-1.5 px-4 py-2.5 bg-primary-500 hover:bg-primary-600 text-white text-sm font-semibold rounded cursor-pointer shadow-xs transition duration-150">
          <Plus className="h-4 w-4" /><span>Register Staff Profile</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="glass-card p-4 rounded flex items-center space-x-3.5">
          ...
          <div><span className="text-[10px] uppercase font-bold text-neutral-400">Total Staff</span><p className="text-xl font-extrabold text-neutral-700">{employees.length}</p></div>
        </div>
        <div className="glass-card p-4 rounded flex items-center space-x-3.5">
          <div className="p-3 bg-primary-50 text-primary-600 rounded"><UserCheck className="h-6 w-6" /></div>
          <div><span className="text-[10px] uppercase font-bold text-neutral-400">Active Staff</span><p className="text-xl font-extrabold text-neutral-700">{employees.filter(e => e.status === 'ACTIVE').length}</p></div>
        </div>
        <div className="glass-card p-4 rounded flex items-center space-x-3.5">
          <div className="p-3 bg-primary-50 text-primary-600 rounded"><Banknote className="h-6 w-6" /></div>
          <div><span className="text-[10px] uppercase font-bold text-neutral-400">Monthly Roster Payroll</span><p className="text-xl font-extrabold text-neutral-700">₹{monthlyPayroll.toLocaleString()}</p></div>
        </div>
      </div>
